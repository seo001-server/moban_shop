...
function formatAmount(minor: number): string {
  if (minor >= 100000) return `${(minor / 100).toFixed(0)}`
  if (minor >= 10000) return `${(minor / 100).toFixed(1)}`
  return `${(minor / 100).toFixed(2)}`
}

function buildFallbackSeries(days: number): DailyAmount[] {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    out.push({ date: `${y}-${m}-${day}`, amount_minor: 0 })
  }
  return out
}

export default function DashboardRevenueChart({ title, series, days, color, colorMuted }: Props) {
  ...
  const maxAmount = useMemo(() => {
    let max = 0
    for (const item of data) max = Math.max(max, item.amount_minor)
    return Math.max(max, 1)
  }, [data])

  const yTicks = useMemo(() => {
    const step = maxAmount <= 500 ? maxAmount : Math.ceil(maxAmount / 4)
    const ticks: number[] = []
    for (let v = 0; v <= maxAmount; v += step) ticks.push(v)
    if (ticks[ticks.length - 1] !== maxAmount) ticks.push(maxAmount)
    return ticks
  }, [maxAmount])
  ...
  const barHeight = (amount: number) => {
    ...
    return Math.max(6, (amount / maxAmount) * chartHeight)
  }
  ...
  return (
    <div className="dashboard-panel">
      <div className="dashboard-panel__head">
        ...
        <p className="dashboard-panel__meta">近 {days} 日趋势（USD 最小单位）</p>
      </div>
      <div className="dashboard-panel__chart-wrap" ref={wrapRef}>
        ...
        >
          ...
          {yTicks.map((tick) => {
            const y = paddingTop + chartHeight - (tick / maxAmount) * chartHeight
            return (
              <g key={tick}>
          ...
          {data.map((item, index) => {
            const x = barX(index)
            const barH = barHeight(item.amount_minor)
            ...
            return (
              <g key={item.date}>
                <rect
                  ...
                  height={barH}
                  rx={4}
                  fill={item.amount_minor > 0 ? color : colorMuted}
                  opacity={item.amount_minor > 0 ? 1 : 0.65}
                />
                ...
                {item.amount_minor > 0 ? (
                  <text x={x + barWidth / 2} y={barY - 5} fill="#475569" fontSize="10" textAnchor="middle">
                    {formatAmount(item.amount_minor)}
                  </text>
                ) : null}
              </g>
            )
